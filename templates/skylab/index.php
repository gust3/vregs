...
if (JRequest::getVar('type','normal')=='search-magaz')
{
...
$db =& JFactory::getDBO();
$query = "SELECT DISTINCT ymname FROM #__vmtools_netshop WHERE ymname like '%".$_GET['str']."%'";
$db->setQuery($query);
$ymnames = $db->loadObjectList();

foreach ($ymnames as $ymname)
{
    $items[$ymname->ymname] = $ymname->ymname;
}
