...
function virtuemartBuildRoute(&$query) {
	...
	switch ($view) {
		...
		case 'productdetails';
			...
			if ($query['layout'] == "otzyvy")
			{
				$segments[]="otzyvy";
				$virtuemart_product_id = $query['virtuemart_product_id'];
				$db =& JFactory::getDBO();
				$query = "SELECT slug FROM #__virtuemart_products_ru_ru WHERE virtuemart_product_id=".$virtuemart_product_id;
				$db->setQuery($query);
				$v_slug = $db->loadResult();
				$segments[] = $v_slug.".html";
			}
			elseif ($query['layout'] == "obzor")
			{
...
function virtuemartParseRoute($segments) {
	...
	if ( $segments[0] == 'otzyvy') {
		
		
		$db =& JFactory::getDBO();
		$query = "SELECT virtuemart_product_id FROM #__virtuemart_products_ru_ru WHERE slug='".str_replace('.html', '', $segments[1])."'";
		$db->setQuery($query);
		$vars['virtuemart_product_id'] = $db->loadResult();
