 ...
 * @version $Id: cart.php 6488 2012-10-01 19:29:08Z Milbo $
 */
	...
	$db =& JFactory::getDBO();
		...
		if ($this->_dataValidated) {
			...
			foreach ($cart->products as $pkey => $prow) {					
				...
				$subject = 'заказ на: '.$prow->product_name;				
				$p = 'product'.$prow->virtuemart_product_id;			
				$array_from_cookie = explode(":", $_COOKIE[$p]);			
				$id_net = $array_from_cookie[1];
				$id_shop = $array_from_cookie[0];
				
				$query = "SELECT email, dostavka FROM #__vmtools_netshop WHERE id=".$id_net;
				$db->setQuery($query);
				...
				if ($id_shop != -1)
				{
				$query = "SELECT adress FROM #__vmtools_shops_adresses WHERE id=".$id_shop;
				$db->setQuery($query);
				$adress = $db->loadResult();
				$adress_text = 'Мне удобно забрать товар по адресу:'.$adress;
				}
				else
				{				
				$id_country = $cart->BT['virtuemart_country_id'];
				$query = "SELECT country_name FROM #__virtuemart_countries WHERE virtuemart_country_id=".$id_country;
				$db->setQuery($query);
				$id_country = $db->loadResult();
				
				$id_state = $cart->BT['virtuemart_state_id'];
				$query = "SELECT state_name FROM #__virtuemart_states WHERE virtuemart_state_id=".$id_state;
				$db->setQuery($query);
				$id_state = $db->loadResult();
				
				$adress_text = 'Я хотел бы чтобы вы мне его доставили по адресу:'.$id_country.', '.$id_state.", ".$cart->BT['address_1'];
				}
				...
				<html>
				<head>
				  <title>Здравствуйте я хотел бы заказать у вас'.$prow->product_name.'</title>
				</head>
				<body>
					
				  Я хотел бы заказать у вас '.$prow->product_name.' - '.$prow->quantity.' шт<br>
				 '.$adress_text.'<br>меня зовут: '.$cart->BT['first_name'].'
				 телефон:'.$cart->BT['phone_1'].'
