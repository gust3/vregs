...
//		if(!empty($product->related) && is_array($product->related) && !empty($product->related[0]))$product_model->addImages($product->related);
	...
	// в зависимости от слоя выставляем тайтл страницы
		switch ($layout) {
		case 'default':
		$document->setTitle("Видеорегистратор ".$product->product_name." - ".strip_tags(($category->category_name ? ($category->category_name . ' - ') : '') . "Видеорегистраторы" ));
		$pathway->addItem("Характеристики");
		break;
		case 'otzyvy':
		$document->setTitle("Отзывы - Видеорегистратор ".$product->product_name);
		$pathway->addItem("Отзывы");
		break;
		case 'obzor':
		$document->setTitle("Обзор - Видеорегистратор ".$product->product_name);
		$pathway->addItem("Обзор");
		break;
		case 'video':
		$document->setTitle("Видео - Видеорегистратор ".$product->product_name);
		$pathway->addItem("Видео");
		break;
		}
	}
	...
	function occurrence_2($ip='', $to = 'utf-8'){
	...
	if (!$_COOKIE['region'])
	{
		...
		if (occurrence_2('','utf-8') == 'Северо-Западный федеральный округ')
		{
			setcookie('region', '2');
			$_COOKIE['region'] = 2;
		}
	...
	if(JRequest::getCmd( 'layout', 'default' )=='notify') $this->setLayout('notify'); //Added by Seyi Awofadeju to catch notify layout
		...
		$db = JFactory::getDBO();
		$query = 'SELECT MAX(price) FROM #__vmtools_nets_products WHERE  id_product='.$virtuemart_product_id;
		$db->setQuery($query);
		$max = $db->loadResult();
		$query = 'SELECT MIN(price) FROM #__vmtools_nets_products WHERE  id_product='.$virtuemart_product_id;
		$db->setQuery($query);
		$min = $db->loadResult();
		$query = 'SELECT MIN(price) FROM #__vmtools_nets_products WHERE  id_product='.$virtuemart_product_id;
		$db->setQuery($query);
		$min = $db->loadResult();
		$query = 'SELECT x,y FROM #__vmtools_regions WHERE  id_region='.$_COOKIE['region'];
		$db->setQuery($query);
		$startpos = $db->loadObject();
		...
		$DATA -> min = $min;
		$DATA -> nets = $nets;
	$this->assignRef('max', $DATA->max);
	$this->assignRef('nets', $DATA->nets);
	$this->assignRef('min', $DATA->min);
	$this->assignRef('startpos', $DATA->startpos);	
	//получаем дополнительные данные в зависимости от слоя
	switch ($layout) {
	case 'otzyvy':
	$query = 'SELECT reiting, plusy, minusy, comment, user, id, parent FROM #__vmtools_products_otzivi WHERE published = 1 AND product_id = '.$virtuemart_product_id;
		$db->setQuery($query);
		$otzyvy = $db->loadObjectList();
		$this->assignRef('otzyvy', $otzyvy);
	break;
	case 'obzor':
		$query = 'SELECT text FROM #__vmtools_opisanie WHERE published = 1 AND product_id = '.$virtuemart_product_id;
		$db->setQuery($query);
		$obzor = $db->loadObjectList();
		$this->assignRef('obzor', $obzor);
	break;
	case 'video':
		$query = 'SELECT url, comment FROM #__vmtools_videos WHERE published = 1 AND product_id = '.$virtuemart_product_id;
		$db->setQuery($query);
	...
	$query = 'SELECT label FROM #__vmtools_regions WHERE   	id_region='.$_COOKIE['region'];
	$db->setQuery($query);
	$regionname = $db->loadResult();
	$this->assignRef('regionname', $regionname);
