 ...
 * @version $Id: view.html.php 6460 2012-09-16 18:43:45Z Milbo $
 */
...
class VirtueMartViewVirtueMart extends VmView {
	...
	public function display($tpl = null) {
		...
		function occurrence2($ip='', $to = 'utf-8'){
		...
		if (!$_COOKIE['region'])
		{
			...
			if (occurrence2('','utf-8') == 'Северо-Западный федеральный округ')
			{
				setcookie('region', '2');
				$_COOKIE['region'] = 2;
			}
		...
		*/
		...
		if(!VmConfig::get('shop_is_offline',0)){
			...
			if (!empty($latest_products_count) and VmConfig::get('show_latest', 1)) {
				$products['new']= $productModel->getProductListing('new', $latest_products_count);
				$productModel->addImages($products['new'],1);
			}
			...
			if (!empty($topTen_products_count) and VmConfig::get('show_topTen', 1)) {
				$products['last']= $productModel->getProductListing('last', $topTen_products_count);
				$productModel->addImages($products['last'],1);
			}
		...
		if(!empty($error)){
			...
			$document->setTitle("Интернет магазин видеорегистраторов Сенсорные Системы");
		}
