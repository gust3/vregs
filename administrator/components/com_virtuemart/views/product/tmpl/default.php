...
<th><?php echo JText::_( 'COM_VIRTUEMART_CATEGORY'); ?></th>
		...
		<th width="40px" ><?php echo $this->sort('product_new', 'COM_VIRTUEMART_PRODUCT_FORM_NEW'); ?>
			 </th>
		<th width="40px" ><?php echo $this->sort('product_last', 'COM_VIRTUEMART_PRODUCT_FORM_LAST'); ?>
			 </th>
		<th width="40px" ><?php echo $this->sort('product_slider', 'COM_VIRTUEMART_PRODUCT_FORM_SLIDER'); ?>
			 </th>
	...
	if ($total = count($this->productlist) ) {
		...
		foreach ($this->productlist as $key => $product) {
			...
			$is_new = $this->toggle($product->product_new, $i, 'toggle.product_new');
			$is_last = $this->toggle($product->product_last, $i, 'toggle.product_last');
			$is_slider = $this->toggle($product->product_slider, $i, 'toggle.product_slider');
			?>
