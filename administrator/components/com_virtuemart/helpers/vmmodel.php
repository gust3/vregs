...
// 		vmdebug('checkFilterDir '.$filter_order_Dir);
	 ...
	 * @author Max Milbers
	 */
	public function getPagination2($total, $limitStart, $perRow = 5) {

			if(empty($this->_limit) ){
				$this->setPaginationLimits();
			}
			$this->_limit = JRequest::getInt('limit', 12);
			$this->_pagination = new VmPagination($total , $limitStart, $this->_limit , $perRow );

// 		}
// 		vmdebug('$this->pagination $total '.$this->_total,$this->_pagination);vmTrace('getPagination');
		return $this->_pagination;
	}
	public function getPagination($limitstart = 0, $perRow = 5) {

			if(empty($this->_limit) ){
				$this->setPaginationLimits();
			}
			$this->_limit = JRequest::getInt('limit', 12);
			$this->_pagination = new VmPagination($this->_total , $limitstart, $this->_limit , $perRow );
